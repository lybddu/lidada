package com.llyb.lidada.model.vo;
...
public class UserAnswerVO implements Serializable {
    ...
    public static UserAnswerVO objToVo(UserAnswer userAnswer) {
        ...
        if (choicesStr != null && choicesStr.startsWith("[")) {
            try {
                choicesList = JSONUtil.toList(choicesStr, String.class);
            } catch (cn.hutool.json.JSONException e) {
                // 处理JSON解析异常，例如记录日志或设置默认值
                e.printStackTrace();
            }
        } else {
            // 处理字符串不是JSON数组格式的情况，例如记录日志或设置默认值
            System.out.println("Choices string is not a valid JSON array: " + choicesStr);
        }

        userAnswerVO.setChoices(choicesList);
        return userAnswerVO;
    }
}